...
/**
 * Service for verifying extracted entities using Perplexity AI
 */
export class PerplexityVerificationService {
  ...
  constructor() {
    ...
    this.apiKey = apiKeyManager.getKeyValue('perplexity') || '';
    this.apiUrl = apiKeyManager.getApiUrl('perplexity') || 'https://api.perplexity.ai/chat/completions';
    this.model = process.env.PERPLEXITY_MODEL || 'sonar-medium-online';
    
    if (!this.apiKey) {
      console.warn('Perplexity API key not configured in PerplexityVerificationService');
    }
  }
